...
LEFT JOIN user_roles ur ON u.id = ur.user_uuid -- Corrigido: u.id já é UUID, ur.user_uuid deve ser UUID
LEFT JOIN student_profiles sp ON u.id = sp.user_uuid -- Corrigido
LEFT JOIN teacher_profiles tp ON u.id = tp.user_uuid -- Corrigido
