-- Script SEGURO para corrigir acesso aos flashcards
...
-- 4. Criar tabela subjects se não existir (estrutura simples)
...
-- 5. Criar tabela topics se não existir
...
-- 6. Inserir dados básicos apenas se não existirem
...
DROP POLICY IF EXISTS "Allow authenticated users to insert flashcards" ON flashcards;
DROP POLICY IF EXISTS "Allow authenticated users to update flashcards" ON flashcards;
DROP POLICY IF EXISTS "Allow authenticated users to delete flashcards" ON flashcards;
...
DROP POLICY IF EXISTS "Allow public read access to topics" ON topics;
