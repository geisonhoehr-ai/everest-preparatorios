...
-- de usuários de teste estejam corretos, incluindo o tratamento de RLS.
...
BEGIN
    ...
    -- 0. Desabilitar RLS e remover políticas temporariamente
    RAISE NOTICE '0. Desabilitando RLS e removendo políticas existentes...';

    -- Desabilitar RLS e remover políticas para student_profiles
    EXECUTE 'ALTER TABLE public.student_profiles DISABLE ROW LEVEL SECURITY';
    RAISE NOTICE '   RLS desabilitado para student_profiles.';
    FOR policy_name IN (SELECT policyname FROM pg_policies WHERE schemaname = 'public' AND tablename = 'student_profiles') LOOP
        EXECUTE 'DROP POLICY ' || quote_ident(policy_name) || ' ON public.student_profiles';
        RAISE NOTICE '   Política % de student_profiles removida.', policy_name;
    END LOOP;

    -- Desabilitar RLS e remover políticas para teacher_profiles
    EXECUTE 'ALTER TABLE public.teacher_profiles DISABLE ROW LEVEL SECURITY';
    RAISE NOTICE '   RLS desabilitado para teacher_profiles.';
    FOR policy_name IN (SELECT policyname FROM pg_policies WHERE schemaname = 'public' AND tablename = 'teacher_profiles') LOOP
        EXECUTE 'DROP POLICY ' || quote_ident(policy_name) || ' ON public.teacher_profiles';
        RAISE NOTICE '   Política % de teacher_profiles removida.', policy_name;
    END LOOP;

    -- Desabilitar RLS e remover políticas para user_roles
    EXECUTE 'ALTER TABLE public.user_roles DISABLE ROW LEVEL SECURITY';
    RAISE NOTICE '   RLS desabilitado para user_roles.';
    FOR policy_name IN (SELECT policyname FROM pg_policies WHERE schemaname = 'public' AND tablename = 'user_roles') LOOP
        EXECUTE 'DROP POLICY ' || quote_ident(policy_name) || ' ON public.user_roles';
        RAISE NOTICE '   Política % de user_roles removida.', policy_name;
    END LOOP;

    -- 1. Corrigir tipo da coluna user_uuid para UUID em tabelas de perfil se for TEXT
    RAISE NOTICE '1. Verificando e corrigindo tipos de user_uuid para UUID...';
    ...
    -- 2. Limpar dados existentes para os emails de teste
    RAISE NOTICE '2. Limpando dados existentes para % e %...', aluno_email, professor_email;
    ...
    -- 3. Criar usuários no auth.users (Supabase Auth)
    RAISE NOTICE '3. Criando usuários de teste no auth.users...';
    ...
    -- 4. Garantir que as colunas de perfil existam
    RAISE NOTICE '4. Verificando e adicionando colunas de perfil (nome_completo, grade, school, subject)...';
    ...
    -- 5. Inserir/Atualizar acesso pago na tabela paid_users
    RAISE NOTICE '5. Inserindo/Atualizando acesso pago na tabela paid_users...';
    ...
    -- 6. Inserir/Atualizar roles na tabela user_roles
    RAISE NOTICE '6. Inserindo/Atualizando roles na tabela user_roles...';
    ...
    -- 7. Inserir/Atualizar perfis específicos
    RAISE NOTICE '7. Inserindo/Atualizando perfis específicos...';
    ...
    -- 8. Recriar políticas de RLS e reabilitar RLS
    RAISE NOTICE '8. Recriando políticas de RLS e reabilitando RLS...';
    ...
    RAISE NOTICE '   Política "Students can manage their profile" recriada.';
    EXECUTE 'ALTER TABLE public.student_profiles ENABLE ROW LEVEL SECURITY';
    RAISE NOTICE '   RLS reabilitado para student_profiles.';

    -- Políticas para teacher_profiles
    CREATE POLICY "Teachers can manage their profile" ON public.teacher_profiles
    FOR ALL USING (auth.uid() = user_uuid);
    RAISE NOTICE '   Política "Teachers can manage their profile" recriada.';
    EXECUTE 'ALTER TABLE public.teacher_profiles ENABLE ROW LEVEL SECURITY';
    RAISE NOTICE '   RLS reabilitado para teacher_profiles.';

    -- Políticas para user_roles
    CREATE POLICY "Users can view and update their own role" ON public.user_roles
    FOR ALL USING (auth.uid() = user_uuid);
    RAISE NOTICE '   Política "Users can view and update their own role" recriada.';
    EXECUTE 'ALTER TABLE public.user_roles ENABLE ROW LEVEL SECURITY';
    RAISE NOTICE '   RLS reabilitado para user_roles.';
