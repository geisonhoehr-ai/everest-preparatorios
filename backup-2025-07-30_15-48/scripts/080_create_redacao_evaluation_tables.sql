...
CREATE TABLE IF NOT EXISTS erros_identificados (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  avaliacao_id UUID REFERENCES avaliacoes_redacao(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_avaliacoes_redacao_id ON avaliacoes_redacao(redacao_id);
CREATE INDEX IF NOT EXISTS idx_avaliacoes_professor_id ON avaliacoes_redacao(professor_id);
CREATE INDEX IF NOT EXISTS idx_erros_avaliacao_id ON erros_identificados(avaliacao_id);
CREATE INDEX IF NOT EXISTS idx_templates_categoria ON templates_feedback(categoria);
