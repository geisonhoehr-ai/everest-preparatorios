-- Script corrigido para configurar storage de avatares
-- Data: 2025-01-28
-- Correção: Cast explícito para UUID nas políticas RLS
...
-- 4. CRIAR POLÍTICAS RLS CORRIGIDAS PARA PROFILES
-- Permitir que usuários vejam seus próprios perfis (com cast explícito)
CREATE POLICY "Users can view own profile" ON public.profiles
  FOR SELECT USING (auth.uid()::uuid = id);

-- Permitir que usuários insiram seus próprios perfis (com cast explícito)
CREATE POLICY "Users can insert own profile" ON public.profiles
  FOR INSERT WITH CHECK (auth.uid()::uuid = id);

-- Permitir que usuários atualizem seus próprios perfis (com cast explícito)
