-- Script SEGURO para reconstruir o sistema de autenticação do zero
...
-- Remover apenas triggers específicos do user_roles
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP TRIGGER IF EXISTS update_user_roles_updated_at ON user_roles;

-- Remover apenas função específica do user_roles
DROP FUNCTION IF EXISTS handle_new_user();

-- NÃO remover update_updated_at_column() pois é usada por outras tabelas
...
-- Trigger para atualizar timestamp (usando função existente)
...
WHERE routine_name IN ('handle_new_user', 'is_admin', 'is_teacher', 'get_user_role')
